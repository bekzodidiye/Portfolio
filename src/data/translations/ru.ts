...
export const ruTranslations: TranslationSchema = {
  ...
  terminal: {
    ...
    bioP2:
      'Успешно выполнил и сдал под ключ более 8 коммерческих проектов на бирже Kwork (Paynet CRM, образовательные боты, торговые утилиты).',
    bioP3:
  ...
  projects: {
    ...
    items: {
      portfolioBot: {
        category: 'Telegram Бот & Mini App Движок',
        summary: 'Официальный интерактивный ассистент с запуском 3D WebApp, мультиязычным каталогом, FSM-формой связи и админ-панелью.',
        features: [
          'Возможность запуска полноэкранного интерактивного 3D портфолио прямо внутри Telegram Mini App.',
          'Модульная архитектура роутеров на aiogram 3.x с поддержкой 3 языков (UZ/RU/EN) и инлайн-меню.',
          'Прямая отправка лидов через машину состояний (FSM) и панель рассылки для администратора.',
        ],
        architecture: 'Асинхронный диспетчер aiogram 3.x, FSM-контекст, Pydantic v2 настройки и интеграция с Telegram WebApp SDK.',
      },
      buddyTeam: {
  ...
  timeline: {
    ...
    workItems: {
      kwork: {
        ...
        badge: '8+ Сданных Проектов',
        period: '2026 – Настоящее время',
        responsibilities: [
          'Самостоятельно спроектировал, протестировал и сдал более 8 сложных backend-проектов под ключ.',
